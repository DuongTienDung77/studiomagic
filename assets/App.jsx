...
// NOTE: This file is intended to be compiled in the browser by Babel Standalone.
// All dependencies like React and GoogleGenAI are expected to be on the `window` object.
...
const buildPresetDirective = (presetId, overrides) => {
...
const getDownloadFilename = (dataUrl) => {
...
const CONTROL_MODES = ['Pose', 'Edge', 'Depth', 'Creative'];
...
const ApiProvider = ({ children }) => {
    ...
    const setAndStoreApiKey = useCallback((key) => {
        setApiKey(key);
        if (key) {
...
const translateText = async (ai, text, sourceLang, targetLang) => {
...
const generateImage = async (ai, parts) => {
  const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash-image-preview',
      contents: { parts },
      config: {
          responseModalities: [window.Modality.IMAGE, window.Modality.TEXT],
      },
  });
...
const generateImageFromText = async (ai, prompt, negativePrompt, config) => {
...
const generateVideo = async (ai, apiKey, prompt, imagePart = null, quality, aspectRatio) => {
    const request = {
...
const resizeImageToAspectRatio = (imageData, targetAspectRatio) => {
    return new Promise((resolve, reject) => {
...
const ApiKeyModal = ({ isOpen, onClose }) => {
    ...
    const handleCheckKey = async () => {
        ...
        try {
            const testAi = new window.GoogleGenAI({ apiKey: keyToCheck });
...
const ImageComparator = ({ beforeSrc, afterSrc }) => {
...
const PoseStudio = ({ onBack }) => {
    ...
    const [beauty, setBeauty] = useState(PRESETS[preset].beauty);
    const [controlMode, setControlMode] = useState('Pose');
                    ...
                    <div>
                        ...
                        <select value={preset} onChange={(e) => setPreset(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text">
                            {Object.entries(PRESETS).map(([key, value]) => (
                                <option key={key} value={key}>{value.label[language]}</option>
                            ))}
                        </select>
                    </div>
                     ...
                     <div>
                        ...
                        <select value={aspectRatio} onChange={(e) => setAspectRatio(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text">
                           {ASPECT_RATIOS.map(ratio => (<option key={ratio.value} value={ratio.value}>{ratio.label[language]}</option>))}
                        </select>
                    </div>

                    <div>
                        <label className="block text-sm font-bold mb-2 text-light-text dark:text-dark-text">{t('outputFormat')}</label>
                        <select value={outputFormat} onChange={(e) => setOutputFormat(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text">
                    ...
                    <div>
                      <label className="block text-sm font-bold mb-2 text-light-text dark:text-dark-text">{t('controlMode')}</label>
                        ...
                        <p className="mt-2 text-xs text-center text-gray-500 dark:text-gray-400 min-h-[2.5em] px-1">
                          {t(`controlModeDesc_${controlMode}`)}
                        </p>
                    </div>
...
const AIStylist = ({ onBack }) => {
    ...
    const [aspectRatio, setAspectRatio] = useState('auto');
    const [outputFormat, setOutputFormat] = useState('image/jpeg');
    const [beauty, setBeauty] = useState('on');
    const [isLoading, setIsLoading] = useState(false);
    const [resultImage, setResultImage] = useState(null);
    const [error, setError] = useState('');
    ...
    useEffect(() => {
        if (prevLangRef.current !== language) {
            setPositivePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['stylistPositiveDefault']
                    ? t('stylistPositiveDefault')
                    : currentPrompt
            );
            setNegativePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['stylistNegativeDefault']
...
const PropFusion = ({ onBack }) => {
    ...
    useEffect(() => {
        if (prevLangRef.current !== language) {
            setPositivePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['propFusionPositiveDefault']
                    ? t('propFusionPositiveDefault')
                    : currentPrompt
            );
            setNegativePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['propFusionNegativeDefault']
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border">
                    ...
                    <div>
                        <label className="block text-sm font-bold mb-2 text-light-text dark:text-dark-text">{t('preset')}</label>
                        <select value={preset} onChange={(e) => setPreset(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text">
                            {Object.entries(PRESETS).map(([key, value]) => (<option key={key} value={key}>{value.label[language]}</option>))}
                        </select>
                    </div>
...
const AIDesign = ({ onBack }) => {
    ...
    useEffect(() => {
        if (prevLangRef.current !== language) {
            setPositivePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['designPositiveDefault']
                    ? t('designPositiveDefault')
                    : currentPrompt
            );
            setNegativePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['designNegativeDefault']
...
const AICreative = ({ onBack }) => {
    ...
    useEffect(() => {
        if (prevLangRef.current !== language) {
            setNegativePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['creativeNegativeDefault']
                ...
                <div className="space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border">
                    ...
                    <div>
                     ...
                     <div>
                        <label className="block text-sm font-bold mb-2 text-light-text dark:text-dark-text">{t('aspectRatio')}</label>
                        <select value={aspectRatio} onChange={(e) => setAspectRatio(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text">
                           {ASPECT_RATIOS.filter(r => r.value !== 'auto').map(ratio => (<option key={ratio.value} value={ratio.value}>{ratio.label[language]}</option>))}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-bold mb-2 text-light-text dark:text-dark-text">{t('outputFormat')}</label>
...
const AIArchitect = ({ onBack }) => {
    ...
    useEffect(() => {
        if (prevLangRef.current !== language) {
            setPositivePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['architectPositiveDefault']
                    ? t('architectPositiveDefault')
                    : currentPrompt
            );
            setNegativePrompt(currentPrompt => 
                currentPrompt === TRANSLATIONS[prevLangRef.current]['architectNegativeDefault']
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border">
                    ...
                    <div>
                        <label className="block text-sm font-bold mb-2 text-light-text dark:text-dark-text">{t('preset')}</label>
                        <select value={preset} onChange={(e) => setPreset(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text">
                            {Object.entries(PRESETS).filter(([key]) => key.startsWith('architecture-')).map(([key, value]) => (<option key={key} value={key}>{value.label[language]}</option>))}
                        </select>
                    </div>
                     <div>
                        <label className="block text-sm font-bold mb-2 text-light-text dark:text-dark-text">{t('aspectRatio')}</label>
...
// Expose main components to the global scope for main.js to render.
